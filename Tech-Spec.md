...
When `STEPFUN_API_KEY` is configured, a small provider adapter may rewrite only the answer text through StepFun Chat Completions. It uses native `fetch`, the fixed HTTPS endpoint `https://api.stepfun.com/v1/chat/completions`, and `STEPFUN_MODEL` with the low-latency default `step-2-mini`. The request contains only the user question plus a bounded summary already present in the caller's role-safe projection: target display name, intent, visible timetable titles/times/categories/statuses, and visible responsibility/care counts. It excludes Evidence text, messages, notification metadata, identifiers not needed for the answer, and conversation history.

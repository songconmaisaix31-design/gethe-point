...
The response is composed only from the caller's `RoleSafeProjection`; targeting another member cannot grant additional access. It may reference timetable item IDs and return fixed UI suggestions. It never returns raw evidence text and never performs a mutation. The response reports `engine: "fixture_intent_router"` so the local capability is not misrepresented as an LLM.
...
- Contract and domain unit tests for validation, authorization, persistence, intent routing, and privacy.

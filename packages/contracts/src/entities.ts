...
export const AcknowledgedCareEventSchema = CareEventBaseSchema.extend({
  ...
  if (event.timedOutAt !== null) {
    if (event.escalationLevel === 0 || event.escalatedAt === null) {
      context.addIssue({
        code: "custom",
        message:
          "A late acknowledgement requires persisted timeout and escalation history",
        path: ["escalatedAt"],
      });
    }
    return;
  ...
  if (event.escalationLevel !== 0 || event.escalatedAt !== null) {
    context.addIssue({
      code: "custom",
      message: "A timely acknowledgement cannot contain escalation history",
      path: ["escalatedAt"],
    });
  }
  if (
    event.acknowledgementDeadline === null ||
    compareTimestamps(event.acknowledgedAt, event.acknowledgementDeadline) !== -1
  ) {
    context.addIssue({
      code: "custom",
      message:
...
export type NewHandledCareEvent = z.infer<typeof NewHandledCareEventSchema>;

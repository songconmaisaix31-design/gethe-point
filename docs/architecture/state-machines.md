...
| `timed_out` | authoritative care time reached the acknowledgement deadline without a timely acknowledgement |
...
| `notified` | `acknowledged` | `AcknowledgeCareEvent` | actor is subject and sampled `CareClock.now()` is strictly before the deadline |
...
| `escalated` | `acknowledged` | `AcknowledgeCareEvent` | subject sends an explicitly late acknowledgement before terminal handling |
...
The general `Clock.now(): Date` port remains available to non-care modules. Care commands use the separate `CareClock.now(): Timestamp` port. Every adapter must validate its source value with `TimestampSchema` before returning; command code preserves the validated string verbatim and never converts it through `Date`, epoch milliseconds, or fixed-precision serialization.

`AcknowledgeCareEvent` samples `CareClock.now()` once. A notified event is timely only when `compareTimestamps(now, acknowledgementDeadline) === -1`; equality is already timed out, so an equal or later timestamp cannot produce the timely acknowledgement transition and permits deterministic timeout. A timely acknowledgement that wins the optimistic-version write before the deadline succeeds; a concurrent tick with the stale version fails. A tick that wins at or after the deadline moves to `timed_out`; a later acknowledgement follows only the explicitly allowed post-escalation late-ack path. No path relies on a caller timestamp, process sleep, or arrival-order assumptions.

For both `AcknowledgeCareEvent` and `HandleCareEvent`, the single validated clock sample is reused verbatim as the transition instant, persisted `acknowledgedAt` or `handledAt`, idempotency claim time, domain-event time, and audit time. An idempotent replay returns the original persisted result and timestamp without sampling a replacement command time.

All persisted comparisons use the package-root `compareTimestamps` function after `TimestampSchema` validation. It orders the proleptic-Gregorian calendar and offset with exact integer arithmetic, then compares fractional digits without rounding or truncation. This keeps microsecond, beyond-nanosecond, cross-day offset, leap-day, long-fraction, and year-`0000` boundaries deterministic without `Date` parsing.

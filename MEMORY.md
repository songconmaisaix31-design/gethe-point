...
- Directory-owner workers default to `setup=skip` so Orca setup cannot race after a clean preflight and drift tracked files. If required JavaScript tools are unavailable, the only allowed bootstrap is `pnpm install --no-lockfile --offline --ignore-scripts` after gate initialization and clean preflight, followed by another preflight before edits; any tracked-file drift must stop and escalate without restoring, staging, or committing unowned files. Coordinator setup remains `run`, and task-level worker setup overrides remain explicit.
...
- Every command that can mutate an existing Run state or its derived STATUS must hold the same bounded cross-process state lock from its first immutable byte snapshot through local commit, evidence generation, Orca mutation, or Worker settlement. Lock timeout fails before any of those effects; never bypass it with a manual state edit.
- Amendment recovery is journal-first: before any mutation, claim one casefold-stable artifact identity per amendment ID and reject namespace orphans or collisions. Each journal-derived record binds its ordinal plus the predecessor amendment ID, journal path, full source hash, and journal byte SHA-256, while the journal persists the immutable post-materialization owned contract. Persistent field projection uses explicit stable order and must pass cross-`PYTHONHASHSEED` two-process crash/replay; replay may restore only missing deterministic derived artifacts from the valid chain.
- Existing wave or Task contract updates must be both never dispatched and transitively downstream in the original DAG from at least one resolution source. `superseded_by` is valid only for a failed source, while `corrected_by` is valid only for a completed source.

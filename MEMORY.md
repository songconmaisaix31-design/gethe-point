...
- Amendment recovery is journal-first. The immutable journal fixes the amendment hash, exact requested changes, before/after state hashes, one `applied_at`, and receipt/STATUS identities before state replacement; replay may reconstruct only missing deterministic derived artifacts from a valid journal and must reject corrupt or conflicting evidence.

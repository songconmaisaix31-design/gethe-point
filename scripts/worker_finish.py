...
def finish(args: argparse.Namespace) -> int:
    ...
    if len(args.summary.strip()) > 1200:
        raise FleetError("--summary must stay under 1200 characters to protect coordinator context")
    if len(args.risks.strip()) > 1000:
        raise FleetError("--risks must stay under 1000 characters to protect coordinator context")
    root = git_root()
    ...
    if not identity["linked"]:
        raise FleetError("worker_finish is forbidden outside a dedicated linked worktree")
    ...
    if branch != ctx.get("branch") or not branch_matches_workspace(branch, str(ctx.get("workspace_name") or "")):
        raise FleetError("current branch no longer matches the assigned worktree identity")
    if not str(ctx.get("run_id") or "").startswith("run_"):
        raise FleetError("task context is missing its Orca Run identity")
    if not isinstance(ctx.get("attempt"), int) or int(ctx["attempt"]) < 1:
        raise FleetError("task context is missing its attempt number")
    if not isinstance(ctx.get("contract_hash"), str) or len(ctx["contract_hash"]) != 64:
        raise FleetError("task context is missing its contract hash")
    ...
    payload = {
        ...
        "worktree_mode": ctx.get("worktree_mode"),
        "branch": branch,
        "base_sha": base_sha,
        "head_sha": head_sha,
        "contract_hash": ctx["contract_hash"],
        "summary": args.summary.strip(),
        "risks": args.risks.strip() or "none",
        "checks": [receipt["command"] for receipt in check_receipts],
        "worktree": identity,
    }
    payload["proof_hash"] = canonical_hash(payload)

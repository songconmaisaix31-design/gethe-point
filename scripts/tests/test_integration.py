...
class IntegrationGateTests(unittest.TestCase):
    ...
    def test_clean_no_ff_merges_pass(self):
        experience = self.worker_branch(
            "trk-experience-expr-001",
            "apps/web/src/components/a.txt",
            "EXPR-001",
        )
        conversation = self.worker_branch(
            "trk-conversation-conv-001",
            "modules/conversation/a.txt",
            "CONV-001",
        )
        git(self.root, "checkout", "-b", "trk-integration-int-001", self.base)
        git(self.root, "merge", "--no-ff", experience, "-m", "merge experience [INT-001]")
        git(self.root, "merge", "--no-ff", conversation, "-m", "merge conversation [INT-001]")
        ...
        result = integration_analysis(
            ...
            self.cfg["tracks"]["integration"]["allow"], [experience, conversation],
        )
        self.assertEqual(result["violations"], [])
        self.assertEqual(set(result["merged_heads"]), {experience, conversation})
    ...
    def test_manual_merge_edit_is_rejected(self):
        ...
        git(self.root, "checkout", "-b", "trk-integration-int-001", self.base)
        git(self.root, "merge", "--no-ff", "--no-commit", experience)
        target = self.root / "apps/web/src/components/a.txt"
        target.write_text("manually edited by integrator\n", encoding="utf-8")
        git(self.root, "add", "apps/web/src/components/a.txt")
        git(self.root, "commit", "-m", "merge experience with edit [INT-001]")

        result = integration_analysis(
            self.root, self.cfg, "integration", self.base, "HEAD",
            self.cfg["tracks"]["integration"]["allow"], [experience],
        )
    ...
    def test_direct_worker_path_commit_is_rejected(self):
        git(self.root, "checkout", "-b", "trk-integration-int-001", self.base)
        target = self.root / "modules/conversation/a.txt"
        target.parent.mkdir(parents=True, exist_ok=True)
        target.write_text("illegal\n", encoding="utf-8")
        git(self.root, "add", "modules/conversation/a.txt")

...
def canonical_hash(value: Any) -> str:
    """Return a stable SHA-256 for task contracts and completion proofs."""
...
def load_config(root: Path, path: Path | None = None) -> dict[str, Any]:
    ...
    if policy and not isinstance(policy, dict):
        raise FleetError("execution_policy must be an object")
    if policy.get("delegate_all_tasks") is True:
        required_mode = str(policy.get("required_worktree_mode") or "new-top-level")
        configured_mode = str(cfg.get("worker_defaults", {}).get("worktree_mode") or "")
        if configured_mode != required_mode:
            raise FleetError(
                "worker_defaults.worktree_mode must equal "
                f"execution_policy.required_worktree_mode ({required_mode})"
            )
        if required_mode != "new-top-level":
            raise FleetError("child-agent isolation requires required_worktree_mode=new-top-level")
        if policy.get("unique_worktree_per_attempt") is not True:
            raise FleetError("delegate_all_tasks requires unique_worktree_per_attempt=true")
        if policy.get("strict_completion") is not True:
            raise FleetError("delegate_all_tasks requires strict_completion=true")
    return cfg
...
def branch_matches_workspace(branch: str, workspace_name: str) -> bool:
    """Accept Orca's optional owner prefix without weakening workspace identity."""
    if not branch or not workspace_name:
        return False
    return branch == workspace_name or re.fullmatch(rf"[^/]+/{re.escape(workspace_name)}", branch) is not None


def worktree_identity(root: Path) -> dict[str, Any]:
    """Describe the current Git worktree and whether it is a linked worktree."""
    git_dir_raw = Path(git_out(root, "rev-parse", "--git-dir"))
    common_raw = Path(git_out(root, "rev-parse", "--git-common-dir"))
    git_dir = (git_dir_raw if git_dir_raw.is_absolute() else root / git_dir_raw).resolve()
...
def validate_plan(plan: Mapping[str, Any], cfg: Mapping[str, Any]) -> list[str]:
    ...
    if policy.get("delegate_all_tasks") is True:
        if not isinstance(execution, dict):
            errors.append("execution must be an object for child-agent-only plans")
            execution = {}
        if execution.get("mode") != "child-agents-only":
            errors.append("execution.mode must be child-agents-only")
        if execution.get("worktree_per_task") is not True:
            errors.append("execution.worktree_per_task must be true")
        if execution.get("reuse_worker") is not False:
            errors.append("execution.reuse_worker must be false")
        if execution.get("coordinator_role") != "control-only":
            errors.append("execution.coordinator_role must be control-only")
    ...
    for wi, wave in enumerate(waves):
        ...
        for ti, task in enumerate(tasks):
            ...
            if policy.get("delegate_all_tasks") is True and track == "control":
                errors.append(f"task {tid}: control track cannot contain plan work; coordinator is control-only")
            required_mode = str(policy.get("required_worktree_mode") or "new-top-level")
            if "worktree_mode" in task and task.get("worktree_mode") != required_mode:
                errors.append(f"task {tid}: worktree_mode must be {required_mode}")
            if task.get("run_in_coordinator") is True:
                errors.append(f"task {tid}: run_in_coordinator is forbidden")
            task_deps[tid] = set(str(x) for x in deps)
            task_tracks[tid] = str(track)
            task_wave[tid] = str(wid)
    ...
    if policy.get("delegate_all_tasks") is True:
        ...
        if not isinstance(completion_task, str) or completion_task not in all_ids:
            errors.append("execution.completion_task must name an existing task")
        else:
            expected = policy.get("completion_task")
            if expected and completion_task != expected:
                errors.append(f"execution.completion_task must be {expected}")
            if task_tracks.get(completion_task) != "integration":
                errors.append("completion task must belong to integration track")
            last_wave_id = str(waves[-1].get("id")) if waves and isinstance(waves[-1], dict) else ""
            if task_wave.get(completion_task) != last_wave_id:
                errors.append("completion task must be in the final wave")
            ...
            visit(completion_task)
            missing = sorted(all_ids - ancestors - {completion_task})
            if missing:
                errors.append(
                    "completion task does not transitively depend on every task: " + ", ".join(missing)
                )
    return errors

...
- The default Orca setup validates the plan only. Shared Git hooks remain opt-in via `python scripts/install_hooks.py`.
- The project-specific example plan mirrors the PRD but is not authorization to launch live runs or workers.
- Review [FLEET_DEPLOYMENT_SPEC.md](FLEET_DEPLOYMENT_SPEC.md) before changing directory ownership or starting orchestration.
...
完整原则见 [`PRINCIPLES.md`](PRINCIPLES.md)，本地验证记录见 [`TEST_REPORT.md`](TEST_REPORT.md)。
...
.agents/
  ...
  plans/example.json         # Wave/Task 计划示例
...
```bash
python scripts/fleet.py validate .agents/plans/example.json
```

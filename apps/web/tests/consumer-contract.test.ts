...
describe("QA-002 narrow HTTP consumer contract", () => {
  ...
  it("accepts a both-confirmed handover while system acceptance is still pending", async () => {
    const bothConfirmed = serverSnapshot({
      revision: 6,
      consent: "shared",
      sharedRows: 1,
      reportRows: 5,
    ...
    const fetcher = vi.fn<typeof fetch>().mockResolvedValueOnce(Response.json(bothConfirmed));
    const client = createHttpExperienceClient({ fetch: fetcher });
    ...
    expect(snapshot).toMatchObject({
      ...
      stageTitle: "双方已确认，等待系统完成交接",
      stageSummary: "双方确认已经记录；系统完成原子接受前，负责人和提醒仍保持原归属。",
      server: {
  ...
  it("reconciles an interrupted acceptance to the both-confirmed pending snapshot", async () => {
    const sourceConfirmed = serverSnapshot({
      revision: 5,
      consent: "shared",
      sharedRows: 1,
      reportRows: 5,
    ...
    const fetcher = vi.fn<typeof fetch>();
    fetcher
      .mockResolvedValueOnce(Response.json(sourceConfirmed))
      .mockRejectedValueOnce(new Error("connection lost after acceptance send"))
      .mockResolvedValueOnce(Response.json(bothConfirmed));
    const client = createHttpExperienceClient({ fetch: fetcher });
    await client.load();

    const error = await client.perform("confirm_handover_to").catch((reason: unknown) => reason);
    ...
    expect((error as Error).message).not.toMatch(/没有提交|状态没有改变|已回滚/);
    expect(requestBodies(fetcher)).toEqual([{ command: "confirm_handover_to" }]);
  });
